...
{
    // MARK: 内中心和外中心
    public var centerInside: CGPoint { get{ return CGPoint(x: size.width / 2, y: size.height / 2) } set{} }
...
{
    //MARK: 圆和半径
    public var radius: CGFloat { get{ return min(width, height) / 2 } set{ width = newValue * 2; height = newValue * 2; circular() } }
    public func circular() -> Void { base.layer.cornerRadius = radius; base.layer.masksToBounds = true }
}
